...
spriteFeaturesFilter.visible = false; // disable features filter for now
	...
	private function convertAndUploadImageOrSprite(fName:String, data:ByteArray):void {
		...
		function decodeError():void {
			DialogBox.notify('Error decoding image', 'Sorry, Scratch (Blue) was unable to load the image '+fName+'.', Scratch.app.stage);
		}
		function spriteError():void {
			DialogBox.notify('Error decoding sprite', 'Sorry, Scratch (Blue) was unable to load the sprite '+fName+'.', Scratch.app.stage);
		}
		var costumeOrSprite:*;
		var fExt:String = '';
	...
	protected function startSoundUpload(sndToUpload:ScratchSound, origName:String, whenDone:Function):void {
		if(!sndToUpload) {
			DialogBox.notify(
					'Sorry!',
					'The sound file '+origName+' is not recognized by Scratch (Blue).  Please use MP3 or WAV sound files.',
					stage);
			return;
		}
		whenDone();
