...
public class Scratch extends Sprite {
	// Version
	public static const versionString:String = '1.2β';
	...
	public var isInDevMode:Boolean = false; // coming soon!!!
...
//		}
	...
	public function setSmallStageMode(flag:Boolean):void {
	...
	protected function addFileMenuItems(b:*, m:Menu):void {
		...
		if (b.lastEvent.shiftKey || (isInDevMode == true)) {
			m.addLine();
	...
	protected function addEditMenuItems(b:*, m:Menu):void {
		...
		if (b.lastEvent.shiftKey || (isInDevMode == true)) {
			m.addLine();
			m.addItem('Developer mode', devMode, true, isInDevMode);
		}
	}
	...
	public function secretDevLogoMenu():void {
		...
		m.addLine();
		m.addItem('Exit dev mode', devMode);
	...
	public function checkIsOffline():Boolean {
		return isOffline;
	}
/*	public function devMode():void {
		if (isInDevMode == false) {
			var d:DialogBox = new DialogBox();
			d.addTitle('Developer Mode');
			d.addField('Password', 120)
			d.setPasswordField('Password', true);
			d.addButton('Submit', checkDevPass);
			d.addButton('Cancel', d.cancel);
			d.showOnStage(stage);
//			DialogBox.notify('Coming soon...', '\nDeveloper mode coming soon...', stage);
		} else {
			DialogBox.notify('Developer Mode', 'Developer mode disabled.', stage);
			isInDevMode = false;
		}
		function checkDevPass():void {
			var d:DialogBox = new DialogBox();
			var devPass:String = d.getField("Password");
			if (devPass == 'megafish') {
				isInDevMode = true;
			} else {
				if (devPass == null) {
					DialogBox.notify('Developer Mode', '\nThere was an error, please try again later.', stage);
				} else {
					DialogBox.notify('Developer Mode', '\nThe password "' + devPass + '" is incorrect.', stage);
				}
			}
		}
	}*/
...
/*	public function primCustomDialog(param1:Function, param2:Function, param3:Function, param4:Function, param5:Function, param6:Function, param7:Function, param8:Function, param9:Function):String {
	...
	public function confirmCloneCountChange(param1:int):void {
		...
		var d:DialogBox = new DialogBox();
		d.addTitle('Maximum Clone Count');
		d.addText('\nThe script is requesting to change the maximum clone count to ' + param1 + '.');
		d.addButton('Accept', confirmChange);
	...
	public function devMode() : void
		...
		babyCheck = function(param1:*):void
			{
			var _loc2_:String = d.getField("Password");
			if(_loc2_.length == 0)
			{
				DialogBox.notify("Developer Mode","Error, no password.");
			}
			else
			{
				checkPass(_loc2_);
	...
	private function checkPass(param1:String):void
	{
		var Password:String = param1;
		if (Password == 'megafish') {
			isInDevMode = true;
		} else {
	...
	public function activateTurboMode():void {
		interp.turboMode = true;
		stagePart.refresh();
	}
